...
<section>
    ...
    <?php $jobile_blog_image = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_id() ), 'jobile-blog-image' );
				    if ($jobile_blog_image[0] != '') { ?><img src="<?php echo esc_url($jobile_blog_image[0]); ?>" width="<?php echo $jobile_blog_image[1]; ?>" height="<?php echo $jobile_blog_image[2]; ?>" alt="<?php the_title(); ?>" />
					<?php } else { ?>
    				...
    				<div class="col-md-10 no-padding-lr">
    				    ...
    				    <div class="col-md-12 no-padding-lr">
    					<p class="result-btm-text"><?php the_excerpt(); ?> <a href="<?php echo esc_url(get_permalink()); ?>" class="color-068587"><?php _e('Read More', 'jobile'); ?></a></p></div>
    				</div>
    			    </div>
    			</div>
            <?php endwhile; ?>
