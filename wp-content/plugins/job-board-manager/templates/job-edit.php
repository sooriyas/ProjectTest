...
if ( ! defined('ABSPATH')) exit;  // if direct access 
	...
	if ( is_user_logged_in()  ) {
		...
		if(!isset($_GET['job_id'])){
			...
			echo '<i class="fa fa-exclamation-circle"></i> '.__('Invalid job id.', 'job-board-manager');
		...
		if($job_bm_can_user_edit_published_jobs=='no'){
			...
			echo '<i class="fa fa-exclamation-circle"></i> '.__('You are not authorized to edit this job.', 'job-board-manager');
			return;
			}
				...
				<div class="job-submit pickform">
					...
					if(wp_verify_nonce( $nonce, 'job_bm_edit' ) && isset($_POST['post_job_hidden'])){
						...
						if(empty($_POST['post_title'])){
							 ...
							 echo '<div class="failed"><b><i class="fa fa-exclamation-circle" aria-hidden="true"></i> '.$job_title['title'].'</b> '.__('missing', 'job-board-manager').'.</div>';
							}
						
						if(empty($_POST['post_content'])){
							
							 $validations['post_content'] = '';
							 echo '<div class="failed"><b><i class="fa fa-exclamation-circle" aria-hidden="true"></i> '.$job_content['title'].'</b> '.__('missing', 'job-board-manager').'.</div>';
							}		
						...
						if($job_bm_reCAPTCHA_enable=='yes'){
							if(empty($_POST['g-recaptcha-response'])){
								 ...
								 echo '<div class="failed"><b><i class="fa fa-exclamation-circle" aria-hidden="true"></i> '.$recaptcha['title'].'</b> '.__('missing', 'job-board-manager').'.</div>';
								}
						...
						if(empty($_POST['job_category'])){
							 ...
							 echo '<div class="failed"><b><i class="fa fa-exclamation-circle" aria-hidden="true"></i> '.$job_category['title'].'</b> '.__('missing', 'job-board-manager').'.</div>';
							}				
						...
						if(empty($validations)){
							...
							echo '<div class="success"><i class="fa fa-check"></i> '.__('Job update successfully.', 'job-board-manager').'</div>';
			...
			/*
						...
						if($step ==1)	{
							...
							echo '<div class="option-title">'.__('Select Category','job-board-manager').'</div>';
							echo '<div class="option-details"></div>';
							
							
							//echo '<p>'.__('Select Category','job-board-manager').'</p>';
							...
							if(!empty($categories)){
								...
								foreach($categories as $category){
									...
									echo '<li ><i class="fa fa-exclamation-triangle"></i> '.__('No categories found.','job-board-manager');
									echo '</li>';
							...
							echo '</div>';
							echo '<a class="steps step-2" href="'.$job_submit_page_url.'?step=2">'.__('Next <i class="fa fa-angle-double-right"></i>','job-board-manager').'</a>';
			...
			*/
			...
			?>
					   <script>
			jQuery(document).ready(function($){jQuery(".meta-fields").steps({ headerTag: ".steps-title", bodyTag: ".steps-body",transitionEffect: "slide",onFinished: function (event, currentIndex){jQuery('.job-submit-button').fadeIn(); },labels: {cancel: "<?php echo __('Cancel','job-board-manager'); ?>",current: "<?php echo __('current step:','job-board-manager'); ?>",pagination: "<?php echo __('Pagination','job-board-manager'); ?>",finish: "<?php echo __('Finish','job-board-manager'); ?>",next: "<?php echo __('Next','job-board-manager'); ?>",previous: "<?php echo __('Previous','job-board-manager'); ?>",loading: "<?php echo __('Loading ...','job-board-manager'); ?>"}}); });
					   </script>
			<?php
			...
			<?php 
				...
				echo '<i class="fa fa-exclamation-circle"></i>'.sprintf(__('You are not authorized to edit this job, please go <a href="%s"><b>account page</b></a> to see your jobs.','job-board-manager'), $login_page_url);
		...
		echo '<i class="fa fa-exclamation-circle"></i>'.sprintf(__('Please <a href="%s"><b>login</b></a> to edit this job.','job-board-manager'),$login_page_url);
