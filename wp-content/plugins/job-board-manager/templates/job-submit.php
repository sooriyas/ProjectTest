	...
	if ( is_user_logged_in() ) 
		{
			...
			if( $job_bm_account_required_post_job=='yes'){
				...
				echo sprintf(__('Please <a href="%s">login</a> to submit job.', 'job-board-manager'), $login_page_url) ;
				return;
				}
		...
		if(wp_verify_nonce( $nonce, 'job_bm_submit' ) && isset($_POST['post_job_hidden'])){
			...
			if(empty($validations)){
				...
				*/ 
				...
				echo '<div class="success"><i class="fa fa-check"></i> '.__('Job Submitted successfully.', 'job-board-manager').'</div>';
				echo '<div class="success"><i class="fa fa-check"></i> '.__('Submission status:', 'job-board-manager').' '.$job_bm_submitted_job_status.'</div>';
				...
				$job_bm_job_expiry_days = get_option('job_bm_job_expiry_days', 30);



				$job_bm_expire_date = date('Y-m-d', strtotime(date('Y-m-d'). ' + '.$job_bm_job_expiry_days.' day'));



				foreach($post_type_input_fields_admin as $key=>$field_data){
					$meta_key = $field_data['meta_key'];

					if($meta_key=='job_bm_job_status'){

						$meta_value = 'open';
                    }
					elseif($meta_key=='job_bm_expire_date'){

						$meta_value = $job_bm_expire_date;
					}
                    elseif($meta_key=='job_bm_featured'){
