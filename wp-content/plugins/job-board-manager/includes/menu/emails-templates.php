	...
	public function job_bm_templates_settings_display(){
		...
		if(empty($_POST['job_bm_hidden']))
			...
			if($_POST['job_bm_hidden'] == 'Y'){
				...
				$html.= '<div class="updated"><p><strong>'.__('Changes Saved.', 'job-board-manager' ).'</strong></p></div>';	
				}
			}
		...
		$html.= '<div class="wrap">';	
		$html.= '<div id="icon-tools" class="icon32"><br></div><h2>'.__(job_bm_plugin_name.' - Emails Templates', 'job-board-manager').'</h2>';	
		...
		$html.= '<p class="submit">
                    <input class="button button-primary" type="submit" name="Submit" value="'.__('Save Changes','job-board-manager' ).'" />
                    <input class="reset-email-templates button" type="button" value="'.__('Reset','job-board-manager' ).'" />					
