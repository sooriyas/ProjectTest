...
class class_job_bm_addons_page  {
	...
	public function job_bm_addons_list_html(){
		...
		foreach($addons_data as $key=>$values){
			...
			$html.= '<div class="meta version"><b>'.__('Version:','job-board-manager').'</b> '.$values['version'].'</div>';
			...
			$html.= '<div class="meta price"><b>'.__('Price:','job-board-manager').'</b> '.$price.'</div>';							
			$html.= '<div class="meta download"><a href="'.$values['item_link'].'">'.__('Download','job-board-manager').'</a></div>';				
...
<div class="wrap">
	...
	<div id="icon-tools" class="icon32"><br></div><?php echo "<h2>".__(job_bm_plugin_name.' - Addons', 'job-board-manager')."</h2>";?>
