...
class job_bm_setup_wizard {
	...
	public function qa_notice_setup_wizard(){
		...
		$notice_action = isset( $_GET['job_bm_welcome_hide'] ) ? sanitize_text_field($_GET['job_bm_welcome_hide']) : '';
		...
		if(  empty($job_bm_welcome) ) {
			...
			<div id="message" class="updated">
				<p><?php echo sprintf(__( '<strong>Welcome to %s</strong> &#8211; Please setup basic settings.', 'job-board-manager' ), job_bm_plugin_name); ?></p>
				<p class="submit">
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=job_bm_welcome' ) ); ?>" class="button-primary"><?php _e( 'Run Wizard Setup', 'job-board-manager' ); ?></a> 
					<a class="button-secondary skip" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'job_bm_welcome_hide', 'hide' ), 'job_bm_welcome_nonce', '_job_bm_welcome_nonce' ) ); ?>"><?php _e( 'Skip Setup', 'job-board-manager' ); ?></a>
				</p>
			</div>
			<?php
		}
	...
	public function setup_options(){
		...
		$options = array(
						...
						'general'=> array(
							'title' => __('General', 'job-board-manager'),
							'options' => array(
								...
								'job_bm_list_per_page' => array(
									...
									'placeholder'=>__('20','job-board-manager'),
									'title'=>__('Post per page ?', 'job-board-manager'),
									'option_details'=>__('Post per page on archive page', 'job-board-manager'),					
								...
								'job_bm_account_required_post_job' => array(
									...
									'required'=>'no', // (yes, no) is this field required.
									'placeholder'=>__('30','job-board-manager'),
									'title'=>__('Account required to post job ?', 'job-board-manager'),
									'option_details'=>__('Only logged-in user can post job', 'job-board-manager'),					
								...
								'job_bm_submitted_job_status' => array(
									...
									'option_details'=>__('New submitted job status ?', 'job-board-manager'),					
									'input_type'=>'select', // text, radio, checkbox, select,
									'input_values'=>array(''), // could be array
									'input_args'=>array('pending'=>__('Pending', 'job-board-manager'), 'publish'=>__('Publish', 'job-board-manager'), 'private'=>__('Private', 'job-board-manager'), 'draft'=>__('Draft', 'job-board-manager')  ), // could be array	
									),									
						...
						'job_post'=> array(
							...
							'options' => array(
								...
								'job_bm_salary_currency' => array(
									...
									'required'=>'no', // (yes, no) is this field required.
									'placeholder'=>__('30','job-board-manager'),
									'title'=>__('Currency symbol ?', 'job-board-manager'),
									'option_details'=>__('Currency symbol ?', 'job-board-manager'),					
									'input_type'=>'text', // text, radio, checkbox, select,
	...
	public function setup_wizard() {
		if ( empty( $_GET['page'] ) || 'job_bm_welcome' !== sanitize_text_field($_GET['page']) ) return;
	...
	public function setup_wizard_body() {
		...
		<html <?php language_attributes(); ?>>
            <head>
                ...
                <title><?php _e( 'Job Board Manager &rsaquo; Setup Wizard', 'job-board-manager' ); ?></title>
                <?php 
...
<?php
	...
	if(!empty($_POST['job_bm_hidden'])){
		...
		elseif($step==2){
			...
			echo '<div class="updated"><p>'.__('Page created.', 'job-board-manager' ).'</p></div>';
        }
		...
		elseif($step==3){
			...
			echo '<div class="updated"><p>'.__('Permissions saved.', 'job-board-manager' ).'</p></div>';
			}		
		...
		elseif($step==4){
			...
			echo '<div class="updated"><p>'.__('Job posting saved.', 'job-board-manager' ).'</p></div>';
			}		
...
?>
			...
			if($step==0){
				...
				$submit_button_text = __('Save options','job-board-manager');
							
	
				echo '<h3 class="">'.__('General options', 'job-board-manager').'</h3>';
			...
			elseif($step==1){
				...
				echo '<h3 class="">'.__('Create pages', 'job-board-manager').'</h3>';



				$submit_button_text = __('Create pages','job-board-manager');
			...
			elseif($step==2){
				...
				echo '<h3 class="">'.__('Save permissions', 'job-board-manager').'</h3>';
				...
				$submit_button_text = __('Save permissions','job-board-manager');
			...
			elseif($step==3){
				...
				echo '<h3 class="">'.__('Job posting', 'job-board-manager').'</h3>';





				$submit_button_text = __('Job posting','job-board-manager');
				...
				$submit_button_text = __('Nothing to Save','job-board-manager');
