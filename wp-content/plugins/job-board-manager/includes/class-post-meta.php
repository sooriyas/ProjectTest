...
<script>
	...
	public function meta_boxes_job($post_type) {
			...
			if (in_array($post_type, $post_types)) {
				...
				add_meta_box('job_metabox',__('job data', 'job-board-manager'), array($this, 'job_meta_box_function'), $post_type, 'normal', 'high');
			}
		}
	...
	public function meta_boxes_job_admin($post_type) {
		...
		if (in_array($post_type, $post_types)) {
			...
			add_meta_box('job_admin_metabox',__('Job Admin', 'job-board-manager'), array($this, 'job_admin_meta_box_function'), $post_type, 'side', 'high');
		}
	}
	...
	public function job_admin_meta_box_function($post) {
		...
		foreach($post_type_input_fields_admin as $option_key=>$field_data){
...
<script>
		...
		echo $html_box;
		//var_dump($post_type_input_fields_admin);
		// Display the form, using the current value.
