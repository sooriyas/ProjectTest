...
function job_bm_registration_validation( $username, $password, $email )  {
    ...
    if ( empty( $username ) || empty( $password ) || empty( $email ) ) {
        $reg_errors->add('field', __('Required form field is missing','job-board-manager'));
    }

    if ( strlen( $username ) < 4 ) {
        $reg_errors->add('username_length', __('Username too short. At least 4 characters is required','job-board-manager'));
    }

    if ( username_exists( $username ) )
        $reg_errors->add('user_name',__( 'Sorry, that username already exists!','job-board-manager'));

    if ( !validate_username( $username ) ) {
        $reg_errors->add('username_invalid', __('Sorry, the username you entered is not valid','job-board-manager'));
    }

    if ( strlen( $password ) < 5 ) {
        $reg_errors->add('password', __('Password length must be greater than 5','job-board-manager'));
    }

    if ( !is_email( $email ) ) {
        $reg_errors->add('email_invalid', __('Email is not valid','job-board-manager'));
    }

    if ( email_exists( $email ) ) {
        $reg_errors->add('email', __('Email Already in use','job-board-manager'));
    }
...
function job_bm_complete_registration() {
    ...
    if ( count($reg_errors->get_error_messages()) < 1 ) {
        ...
        echo __('Registration complete.','job-board-manager');   
	}
}
