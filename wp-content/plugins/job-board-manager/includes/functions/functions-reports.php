...
//var_dump(sizeof($wp_query));
	...
	echo '<p>'.__('Job posting last 7 days.','job-board-manager').'</p>';
...
function job_bm_report_html_job_display(){
	...
	if($range=='custom'){
		...
		if(isset( $_GET['before'])){
			$before_date = sanitize_text_field($_GET['before']);
			}
		else{
			$before_date = '';	
			}
		...
		if(isset( $_GET['after'])){
			$after_date = sanitize_text_field($_GET['after']);
			}
		else{
			$after_date = '';	
			}			
		...
		'.__('Custom:','job-board-manager').'
			...
			<input class="button" value="'.__('Go','job-board-manager').'" type="submit">
		</form>
