...
function job_bm_ajax_delete_attachment(){
		...
		$attach_id = (int)sanitize_text_field($_POST['attach_id']);
...
add_shortcode('job_bm_ajax_update_fields','job_bm_ajax_update_fields');
	...
	function job_bm_ajax_update_fields() {
		...
		echo '<i class="fa fa-check"></i> '.__('Update Done','job-board-manager');
	...
	function job_bm_ajax_reset_fields() {
		...
		//echo 'Reset Done';
		echo '<i class="fa fa-check"></i> '.__('Reset Done','job-board-manager');
		die();
		}
