			...
			$class_job_bm_error_log = new class_job_bm_error_log();
					...
					foreach($meta_fields_data as $key=>$meta_fields_group){
						...
						foreach($meta_fields as $option_key=>$option_info){
							...
							if($success){
								$error_log_data[] = array('type'=>'add_meta','status'=>'success','message'=> __(sprintf('%s - Job post meta added successful',$job_ID),'job-board-manager'));
								}
							else{
								$error_log_data[] = array('type'=>'add_meta','status'=>'failed','message'=>__(sprintf('%s - Job post meta added failed',$job_ID),'job-board-manager'));
								}
